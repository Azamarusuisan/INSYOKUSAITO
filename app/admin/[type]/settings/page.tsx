...
export default function SettingsPage({
  ...
  const [name, setName] = useState(shop.info.name);
  const [nameJa, setNameJa] = useState(shop.info.nameJa);
  const [tagline, setTagline] = useState(shop.info.tagline);
  ...
  const isDirty =
    name !== shop.info.name ||
    nameJa !== shop.info.nameJa ||
    tagline !== shop.info.tagline;
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <header>
        ...
        <p className="mt-1 text-xs text-neutral-600 sm:text-sm">
          お客様画面のヘッダーに表示される情報です。
        </p>
      </header>

      <form
        onSubmit={onSave}
        className="space-y-4 rounded-lg border border-neutral-200 bg-white p-5 sm:p-6"
      >
        <Field
          label="店名（英語・サブ表記）"
          required
          error={errors.name}
          hint="例: Trattoria Sole / Maison Lumière"
        >
          <Input
            type="text"
            value={name}
